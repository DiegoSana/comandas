...
class CategoriasController extends Controller
{
	...
	public function accessRules()
	{
        ...
        protected function checkCategoriaPermiso($idCat)
        {
		$cat = Categorias::model()->findByPk($idCat);
                $this->checkAplicacion($cat->aplicacion_id);
        }
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Categorias']))
		{
			...
			if($model->save()){
                            Yii::app()->user->setFlash('success', "La categoría \"".$model->nombre."\" se ha creado correctamente!");
	...
	{
		$model=$this->loadModel($id);
                $this->checkCategoriaPermiso($model->id);
		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);
		...
		if(isset($_POST['Categorias']))
		{
			$model->attributes=$_POST['Categorias'];
                        $this->checkAplicacion($model->aplicacion_id);
			if($model->save()){
                            Yii::app()->user->setFlash('success', "La categoría \"".$model->nombre."\" se ha actualizado correctamente!");
                            $this->redirect(array('view','id'=>$model->id));
                        }
		}
