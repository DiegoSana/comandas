...
class WSAA {
  ...
  /*
   * Certificado .crt de acceso al ws (The X.509 certificate in PEM format)
   */
  public $cert;
  
  /*
   * Llave privada del certificado .crt (The private key correspoding to CERT (PEM))
   */
  public $key;
  ...
  public function __construct($config) 
  {
    ...
    $this->key = $config['key'];
    $this->passphrase = $config['passphrase'] ? $config['passphrase'] : '';
    ...
    if (!file_exists($this->path.$this->key)) $this->error[] = " Failed to open ".$this->key;
  ...
  {
    $STATUS = openssl_pkcs7_sign($this->path."xmlgenerados/TRA.xml", $this->path."xmlgenerados/TRA.tmp", "file://".$this->path.$this->cert,
      array("file://".$this->path.$this->key, $this->passphrase),
      array(),
      !PKCS7_DETACHED
    );
